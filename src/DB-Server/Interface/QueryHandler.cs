...
public class QueryHandler
{
    ...
    {
        //_logger.LogTrace("Received query: {Query}", query);
        try
        {
            ...
            if (tx.Status == TransactionStatus.LocalCompleted || tx.Status == TransactionStatus.DidNotExecute)
            {
                ...
                if (!_transactionWaitHandles.TryAdd(tx.Tid, new TaskCompletionSource<(bool, string)>(TaskCreationOptions.RunContinuationsAsynchronously)))
                {
    ...
    {
        persistentStorage.SaveStorageToDisk("/home/ubuntu/minerva_data/");
