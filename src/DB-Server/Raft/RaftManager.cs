...
public class RaftManager
{
    ...
    private readonly SemaphoreSlim _leaderChangeLock = new(1, 1);
    ...
    public RaftManager(Node[] nodes)
    {
        ...
        foreach (var node in nodes)
        {
            if (node.IsSelfNode)
            {
                ...
                {
                    RequestTimeout = TimeSpan.FromMilliseconds(280),
    ...
    public async Task LeaderReplicate(List<int> indices, int epochId)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Current node is not the leader, cannot replicate");
                return;
            }
