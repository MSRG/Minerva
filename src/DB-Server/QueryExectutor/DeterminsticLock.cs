...
public class DeterminsticLockManager
{
    ...
    public int Lock(TransactionRecord tx)
    {
        ...
        GetWriteLock(tx, tx.WriteSet.KVWriteSet.Keys, _lockTables.YCSBLockTable, ref notAcquired);
        ...
        GetReadLock(tx, tx.ReadSet.KVReadKeys, _lockTables.YCSBLockTable, ref notAcquired);
    ...
    public void Release(TransactionRecord tx)
    {
        lock (this)
        {
            ReleaseLocks(tx, tx.WriteSet.KVWriteSet, tx.ReadSet.KVReadKeys, _lockTables.YCSBLockTable);
    ...
    public void ReleaseLocks<Tkey, TValue>(TransactionRecord tx, IDictionary<Tkey, TValue> writeSet, List<Tkey> readSet, Dictionary<Tkey, LinkedList<LockRequest>> table)
    {
    ...
    private void ReleaseKey<TKey>(TKey key, TransactionRecord tx, IDictionary<TKey, LinkedList<LockRequest>> table)
    {
