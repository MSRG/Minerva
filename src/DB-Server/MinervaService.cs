...
public class MinervaService : IDisposable
{
    ...
    public IDBInterface Interface { get; private set; }
    ...
    public MinervaService(MinervaConfig config, NodeInfo[] nodes)
    {
        ...
        _server = new Server(_cluster.SelfNode.Port, _logReceiveHandler, _clientInterface);
    ...
    {
        Interface = interfaceType switch
        {
            InterfaceType.CLI => CLIInterface.GetInstance(_queryHandler),
            InterfaceType.TCP => _clientInterface,
        ...
        Running = true;
        //Stats.StartThroughputReporting();
        Interface.Start();
        _logger.LogInformation("Minerva interface stopped");
        //Stats.StopThroughputReporting();
