...
public class DeterministicTx : Transaction
{
    ...
    private readonly WriteSet WriteSet = new();
    ...
    {
        Put<(int, string), string>(WriteSet.KVWriteSet, (shard, key), value);
    }

    public override bool KeyGet(int shard, string key, out string value)
    {
        return Get(WriteSet.KVWriteSet, Database.KV, (shard, key), out value);
    ...
    public override bool GetCustomerByLastName(int warehouseId, int districtId, string lastName, out Customer customer)
    {
        ...
        {
            var cl = customerList.ToList();
            cl.Sort((a, b) => a.CID.CompareTo(b.CID));

            int middleIndex = cl.Count / 2;
            if (cl.Count % 2 == 0)
            {
                middleIndex--;
            }
            GetCustomerByID(cl[middleIndex], out customer);
            return true;
        }
    }
    ...
    private static void Put<TKey, TValue>(IDictionary<TKey, TValue> dictionary, TKey key, TValue value)
    {
        dictionary[key] = value;
    }
    
    private bool Get<TKey, TValue>(IDictionary<TKey, TValue> writeSet, Database database, TKey key, out TValue value)
    {
        ...
        else if (_persistDb.Get<TKey, TValue>(database, key, out PersistEntry<TValue> entry))
        {
            value = entry.Value;
            return true;
        }
