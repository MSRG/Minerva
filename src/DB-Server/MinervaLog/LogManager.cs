...
public class GlobalLogManager
{
    ...
    public const bool SIM_RAFT = true;
    public const int RTT = 0;
    ...
    public GlobalLogManager(MinervaConfig config, Cluster cluster, PersistentStorage persistedDB, MinervaTxOCCExecutor occExecutor, DeterministicExecutor deterministicExecutor, ILogConnections logConnection, RaftManager raftManager)
    {
        ...
        RaftNode.OnLeaderChanged += SetCoordinator;
    ...
    public void SetCoordinator(RaftClusterMember leader)
    {
        ...
        if (isLeader)
        {
            _logger.LogInformation("This node is elected as the new Raft leader");
            IsCoordinator = true;
        }
        else
        {
        ...
        while (_running)
        {
            ...
            (DateTime.Now - _lastGlobalCommitTime).TotalMilliseconds > _config.CoordinatorGlobalEpochInterval)
            {
                ...
                if (flag)
                {
                    ...
                    {
                        RaftNode.RaftLog.AppendLog([.. tmpPoAIndices], CurrentEpochId, RTT);
    ...
    private void CommitHandlerThread()
    {
        while (_running)
        {
            ...
            try
            {
                ...
                int commitTo = RaftNode.RaftLog.Log.Count;
    ...
    private Channel<WriteSet> _applyPDBChannel;
    public void GlobalCommit(int[] ConsistentCutIndices, int epochId)
    {
        ...
        {
            async Task ApplyPersistedDBThread(int epochId)
            {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < GlobalLogs.Length; i++)
            {
                ...
                for (int j = LastCommittedIndices[i] + 1; j <= ConsistentCutIndices[i]; j++)
                {
                    ...
                    {
                        //_logger.LogTrace("[RequestMissingBatches] Missing batch {ReplicaId}:{BatchId}", i, j);
