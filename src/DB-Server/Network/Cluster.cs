...
public class Cluster : IDisposable
{
    ...
    public async Task BroadcastMessage<T>(T message, bool includeSelf, ServerMessageTypes streamToUse)
    {
        try
        {
            Task[] sendTasks = new Task[Nodes.Length];
            ...
            for (int i = 0; i < Nodes.Length; i++)
            {
                ...
                {
                    sendTasks[i] = node.Sender.SendMessageBytes(memoryOwner.Memory, streamToUse);
                }
                else
                {
        ...
        {
            _logger.LogError("Error broadcasting message of type {MessageType}: {ErrorMessage} {StackTrace}", typeof(T).Name, ex.Message, ex.StackTrace);
            throw;
        }
    ...
    public async Task BroadcastMessageWithoutFieldNumber<T>(T message, bool includeSelf, ServerMessageTypes streamToUse)
    {
        try
        {
            Task[] sendTasks = new Task[Nodes.Length];
