...
public class MessageProcessorGeneric<T>
{
    ...
    private readonly PipeReader _reader;
    private readonly Stream _stream;
    private readonly IReceivedRequestHandler<T> _receivedRequestHandler;
    private readonly ILogger _logger = LoggerManager.GetLogger();

    public MessageProcessorGeneric(Stream stream, IReceivedRequestHandler<T> requestHandler)
    {
        _stream = stream;
        _reader = PipeReader.Create(stream);
        _receivedRequestHandler = requestHandler;
                ...
                try
                {
                    while (TryReadMessage(ref buffer, ref position, out var message))
                    {
                        ...
                        if (message != null && !cancellationToken.IsCancellationRequested)
                        {
                            ...
                            _ = Task.Run(() => _receivedRequestHandler.HandleMessage(message), cancellationToken).ContinueWith(t =>
                            {
                                if (t.IsFaulted)
                                {
                                    var ex = t.Exception?.GetBaseException();
                                    _logger.LogError($"Error handling message of type {typeof(T).Name}: {ex?.Message} {ex?.StackTrace}");
                                }
...
public static class ProtoMessageHelper
{
    ...
    public static MemoryOwner<byte> SerializeToMemoryOwner<T>(T message)
    {
        ...
        {
            Console.WriteLine($"Serialization error: {ex.Message}, {ex.StackTrace}");
            throw;
        }
