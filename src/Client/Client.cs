...
public class ClientResponseHandler : IReceivedRequestHandler<TxResult>
{
    ...
    public void HandleMessage(TxResult message)
    {
        ArgumentNullException.ThrowIfNull(message);


        if (_pendingRequests.TryRemove(message.SeqId, out var tcs))
        {
            tcs.SetResult(message);
        }
        else
        {
            throw new InvalidOperationException($"No pending request found for SeqId {message.SeqId}");
        }
